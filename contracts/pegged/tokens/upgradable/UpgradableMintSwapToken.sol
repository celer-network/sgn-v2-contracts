...
import "./UpgradableMintToken.sol";

/**
 * @title Canonical token that supports multi-bridge minter and multi-token swap
 */
contract UpgradableMintSwapToken is Initializable, UpgradableMintToken {
    ...
    function initialize(
        ...
        uint8 decimals_) public override initializer {
            UpgradableMintToken.initialize(name_, symbol_, decimals_);
    }
    ...
    function swapBridgeForCanonical(address _bridgeToken, uint256 _amount) external returns (uint256) {
        ...
        IERC20Upgradeable(_bridgeToken).safeTransferFrom(msg.sender, address(this), _amount);
        return _amount;
    }
    ...
    function swapCanonicalForBridge(address _bridgeToken, uint256 _amount) external returns (uint256) {
        ...
        IERC20Upgradeable(_bridgeToken).safeTransfer(msg.sender, _amount);
        return _amount;
    }
