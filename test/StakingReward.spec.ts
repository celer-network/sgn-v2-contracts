...
describe('StakingReward Tests', function () {
  ...
  it('should claim reward successfully', async function () {
    ...
    r = await getStakingRewardRequest(chainId, reward.address, validators[0].address, parseUnits('90', 'wei'), signers);
    await expect(reward.claimReward(r.rewardBytes, r.sigs))
      .to.emit(reward, 'StakingRewardClaimed')
  ...
  it('should fail to claim reward if there is no new reward', async function () {
    const r = await getStakingRewardRequest(chainId, reward.address, validators[0].address, parseUnits('0'), signers);
  ...
  it('should fail to claim reward with insufficient signatures', async function () {
    const r = await getStakingRewardRequest(chainId, reward.address, validators[0].address, parseUnits('10', 'wei'), [
      signers[0],
      signers[1]
    ]);
